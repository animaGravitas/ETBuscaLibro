    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if form.cleaned_data.get('send_to_all'):
                # Obtener la lista de todos los correos electrónicos de los usuarios registrados
                receivers = [user.email for user in get_user_model().objects.all()]
            else:
                receivers = form.cleaned_data.get('receivers').split(',')

            mail = EmailMessage(subject, email_message, f"BuscaLibro <{request.user.email}>", bcc=receivers)
            mail.content_subtype = 'html'

            if mail.send():
                messages.success(request, "Correo enviado satisfactoriamente")
            else:
